...
SampleProducerThread::SampleProducerThread() : m_source(nullptr), m_running(false), m_stop(false) {}
...
void SampleProducerThread::addBuffer(SampleBuffer* buffer, bool enableBuffer) {
  Lock lock(m_mutex);
...
void SampleProducerThread::remBuffer(SampleBuffer* buffer) {
  Lock lock(m_mutex);
  const auto it = std::find_if(m_buffers.begin(), m_buffers.end(), [buffer](const buffer_t& b) { return b.buffer == buffer; });
  if (it != m_buffers.end()) {
    m_buffers.erase(it);
  }
}
  ...
  for (const buffer_t& buffer : m_buffers) {
    if (buffer.enabled) {
      ...
      while (buffer.buffer->avail() < MIN_BUFFER_SAMPLES) {
        ...
        sbl.unlock();
        const int samples = m_source->readSamples(this);
        if (samples < 0) {
          return false;
        }
        if (samples == 0) {
